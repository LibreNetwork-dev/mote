self.html = function (strings, ...values) {
  ...
  function normalizeNodes(val) {
    ...
    if (val instanceof DocumentFragment) return Array.from(val.childNodes);
    if (val instanceof NodeList || val instanceof HTMLCollection)
      return Array.from(val);
    if (Array.isArray(val)) return val.flatMap(normalizeNodes);
  ...
  const htmlString = strings
    .map((str, i) => {
      ...
      // nodes, fragments, NodeLists, arrays
      if (
        val instanceof Node ||
        ...
        const nodes = normalizeNodes(val); // <-- no cloning
        markers.push(nodes);
        return str + marker;
      }
      ...
      if (val && typeof val === "object" && "subscribe" in val) {
        ...
        val.subscribe((v) => {
          const nodes = normalizeNodes(v); // <-- no cloning
